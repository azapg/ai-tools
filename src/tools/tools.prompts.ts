...
const KEY_POINTS = (topic: string) => `Escribe los 5 puntos claves a la hora de estudiar sobre ${topic}. Los puntos deben ser una oración y fáciles de comprender. También deben sugerir nuevas ideas para poder investigarlas y aprender sobre ellas. Incluye solo lo más importante. Recuerda que los puntos son para una persona que no sabe nada del tema.`;
const SIMPLE_EXPLANATION = (topic: string) => `Explica ${topic} de manera simple, intenta usar analogías o ejemplos para mejorar la explicación. Intenta explicarlo de manera que un niño de 7 años podría entenderlo.`;
const ANSWERING_BOT = (question: string) => `Soy un robot responde preguntas extremadamente inteligente. ¡Pregúntame lo que sea, siempre tengo una respuesta! Respondo con sarcasmo o de manera amigable :).\n\nPregunta: ${question}\nRespuesta:`;
const EXAMN_QUESTIONS = (subject: string, grade: number, topic: string) => `Crea un examen de 10 preguntas de ${subject} sobre ${topic} para un grupo de ${grade}° Grado. En español.\n\nEmpieza.`;
